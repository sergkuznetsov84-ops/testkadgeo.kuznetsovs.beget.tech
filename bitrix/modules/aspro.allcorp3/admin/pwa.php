...
$code = 'ALLCORP3';
$moduleID = 'aspro.'.str_replace('_', '', strtolower($code));
$modulePath = str_replace('.', '/', $moduleID);

$RIGHT = $APPLICATION->GetGroupRight($moduleID);

$APPLICATION->SetTitle(Loc::getMessage('ASPRO_'.$code.'_PWA'));?>
<?
if (!Loader::includeModule($moduleID)) {
	CAdminMessage::ShowMessage(Loc::getMessage('ASPRO_'.$code.'_CANT_INCLUDE_MODULE'));
...
<?if($RIGHT >= 'R'):?>
	<?
	$GLOBALS['APPLICATION']->SetAdditionalCss('/bitrix/css/'.$modulePath.'/style.css');
				...
				foreach(PWA::getParams() as $blockCode => $arBlock){
					foreach($arBlock['OPTIONS'] as $optionCode => $arOption){
						...
						elseif($optionType === 'file'){
							...
							$newVal = Solution::unserialize(Option::get($moduleID, $optionCode, $arValueDefault, $optionsSiteID));
							...
							if(isset($_FILES[$optionCode.'_'.$optionsSiteID]) && (strlen($_FILES[$optionCode.'_'.$optionsSiteID]['tmp_name']['n0']) || strlen($_FILES[$optionCode.'_'.$optionsSiteID]['tmp_name']['0']))){
								...
								if(file_exists($absFilePath)){
									...
									if($fileID = CFile::SaveFile($arFile, $moduleID)){
										$arValues[] = $fileID;
									}
	...
	?>
	<form method="post" class="lite_options" enctype="multipart/form-data" action="<?=$APPLICATION->GetCurPage()?>?mid=<?=urlencode($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
		<?=bitrix_sessid_post();?>
		<?$tabControl->Begin();?>
		<?foreach ($arTabs as $key => $arTab):?>
		...
		<input <?if($RIGHT < "W") echo "disabled"?> type="submit" name="Apply" class="submit-btn" value="<?=GetMessage('MAIN_SAVE')?>" title="<?=GetMessage('MAIN_SAVE')?>">

		<?$tabControl->End();?>
	</form>
<?else:?>
	<?=CAdminMessage::ShowMessage(Loc::getMessage('ASPRO_'.$code.'_ACCESS_DENIED'));?>
<?endif;?>
