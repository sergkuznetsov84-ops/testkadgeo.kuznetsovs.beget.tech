...
if($_SERVER['REQUEST_METHOD'] === 'GET'){
	$GLOBALS['APPLICATION']->SetTitle(GetMessage('ALLCORP3_CONTROL_CENTER_TITLE'));
}

$RIGHT = $GLOBALS['APPLICATION']->GetGroupRight(Solution::moduleID);
if($RIGHT >= 'R'){
	if(CModule::IncludeModule(Solution::moduleID)){
		if($_SERVER['REQUEST_METHOD'] === 'GET'){
			CJSCore::Init(array('jquery'));
			?>
			<link href="/bitrix/css/<?=Solution::moduleID;?>/style.css" type="text/css" rel="stylesheet" />
			...
			$.ajax({
				...
				error: function(){
					...
					$('#aspro_admin_area').append('<div class="adm-info-message-wrap adm-info-message-red"><div class="adm-info-message"><div class="adm-info-message-title"><?=GetMessage('ALLCORP3_MODULE_CONTROL_CENTER_ERROR')?></div><div class="adm-info-message-icon"></div></div></div>');
				}
			});
			</script>
			<?
		...
		CAdminMessage::ShowMessage(GetMessage('ALLCORP3_MODULE_NOT_INCLUDED'));
	}
}
else{
	CAdminMessage::ShowMessage(GetMessage('ALLCORP3_NO_RIGHTS_FOR_VIEWING'));
}
?>
<?
if($_SERVER['REQUEST_METHOD'] === 'GET'){
