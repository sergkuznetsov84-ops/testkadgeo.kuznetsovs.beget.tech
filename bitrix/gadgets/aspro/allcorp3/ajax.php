...
if($_REQUEST && isset($_REQUEST['mid'])){
	if(strlen($moduleID = htmlspecialcharsbx($_REQUEST['mid']))){
		...
		$langValue = (isset($_REQUEST['lid']) && $_REQUEST['lid'] ? $_REQUEST['lid'] : LANGUAGE_ID);
		__IncludeLang(dirname(__FILE__).'/lang/'.$langValue.'/index.php');
		...
		<?if($arInfo):?>
			...
			<?if($bSupportLess14 || !$bSupportActive):?>
				...
				<div class="aspro-gadget-bottom">
					<a class="aspro-gadgets-button" href="/bitrix/admin/aspro.<?=$moduleID?>_mc.php?lang=<?=htmlspecialcharsbx($_REQUEST['lid']);?>">
						<div class="aspro-gadgets-button-lamp"></div>
						<div class="aspro-gadgets-button-text"><?=GetMessage('GD_ASPRO_GET_MORE')?></div>
					</a>
					<a class="aspro-gadgets-button aspro-gadgets-button-buy" href="https://aspro.ru/shop/solutions/prodleniya/?utm_source=siteclient&utm_medium=admin_panel&utm_campaign=aspro_warm_prodlenie" target="_blank">
			...
			<?endif;?>
		<?else:?>
			<?$message = $arUpdateList && isset($arUpdateList['ERROR']) ? $arUpdateList['ERROR'][0]['#'].' ('.$arUpdateList['ERROR'][0]['@']['TYPE'].')' : GetMessage('GD_ASPRO_ERROR');?>
			<div class="aspro-gadgets-title2 pink" title="<?=htmlspecialcharsbx($message)?>"><?=$message?></div>
		<?endif;?>
		<?
	}
}
