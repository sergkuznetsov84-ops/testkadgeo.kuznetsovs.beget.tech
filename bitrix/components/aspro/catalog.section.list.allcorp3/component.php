...
else
{
	...
	if(!is_array($arrFilter))
		$arrFilter = array();
	if($arParams['FRONT_PAGE'] && $arParams['SHOW_CHILDS'] && isset($arrFilter["UF_SHOW_ON_INDEX_PAG"])){
		unset($arrFilter['UF_SHOW_ON_INDEX_PAG']);	
	}
}

$arrFilter['IBLOCK_ID'] = $arParams['IBLOCK_ID'];
TSolution::makeSectionFilterInRegion($arrFilter);
...
if (TSolution\Product\Common::checkCatalogGlobalFilter()) {
	$arrElementsFilter = array_merge($GLOBALS['arRegionLink'], [
...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arrFilter, "COMPACT_MOBILE_".$arTheme["CATALOG_SECTIONS_COMPACT_VIEW_MOBILE"]["VALUE"])))
{
	...
	//LIMIT
	$bNewVersion = false;
	if($info = CModule::CreateModuleObject('iblock'))
	{
		if(!CheckVersion('15.5.4', $info->MODULE_VERSION))
			$bNewVersion = true;
	}
	$arParams['SECTION_COUNT'] = (!isset($arParams['SECTION_COUNT']) || !$arParams['SECTION_COUNT'] ? 4 : $arParams['SECTION_COUNT']);
	$arNavs = ($bNewVersion ? array('nTopCount' => $arParams['SECTION_COUNT']) : false);

	//EXECUTE
	$rsSections = CIBlockSection::GetList($arSort, array_merge($arFilter, $arrFilter), $arParams["COUNT_ELEMENTS"], $arSelect, $arNavs);
	$rsSections->SetUrlTemplates("", $arParams["SECTION_URL"]);
	...
	while($arSection = $rsSections->GetNext())
	{
		...
		if ($arParams["COUNT_ELEMENTS"] && !empty($arrElementsFilter)) {
			$arrElementsFilterCNT = array_merge($arrElementsFilter, array("SECTION_ID" => $arSection["ID"]));
			if ($arSection['RIGHT_MARGIN'] == ($arSection['LEFT_MARGIN'] + 1)) {
				$arrElementsFilterCNT['INCLUDE_SUBSECTIONS'] = 'N';
			}
	...
	if($arResult["SECTIONS"] && !$bNewVersion)
	{
		$arResult["SECTIONS"] = array_slice($arResult["SECTIONS"], 0, $arParams['SECTION_COUNT']);
	}
