...
if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get(VENDOR_MODULE_ID, 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
{
	$arResult['REGIONS'] = TSolution\Regionality::getRegions();
	$arResult['CURRENT_REGION'] = TSolution\Regionality::getCurrentRegion();
	$arResult['REAL_REGION'] = TSolution\Regionality::getRealRegionByIP();
...
if($arResult['REGIONS'])
{
	...
	if($arResult['POPUP'])
	{
		...
		if($arSectionsID)
		{
			$arSections = TSolution\Cache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => TSolution\Cache::GetIBlockCacheTag(TSolution\Cache::$arIBlocks[SITE_ID][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regionality'][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => TSolution\Cache::$arIBlocks[SITE_ID][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regionality'][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
			if($arSections)
			{
				$arSections2 = array();
				foreach($arSections as $key => $arSection)
				{
					...
					if($arSection['DEPTH_LEVEL'] > 1)
					{
						...
						$arTmpSection = TSolution\Cache::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => TSolution\Cache::GetIBlockCacheTag(TSolution\Cache::$arIBlocks[SITE_ID][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regionality'][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => TSolution\Cache::$arIBlocks[SITE_ID][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regionality'][VENDOR_PARTNER_NAME . '_'. VENDOR_SOLUTION_NAME . '_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
...
elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get(VENDOR_MODULE_ID, 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
{
	$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
	$this->IncludeComponentTemplate();
}
