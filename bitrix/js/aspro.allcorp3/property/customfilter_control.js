function initAsproAllcorp3CustomFilterControl(params)
{
	var data = JSON.parse(params.data);
	if (data)
	{
		window['aspro_Allcorp3_customfilter_conditions_' + params.propertyID] = new AsproAllcorp3CustomFilterControl(data, params);
	}
}

function AsproAllcorp3CustomFilterControl(data, params)
{
	...
	BX.addCustomEvent('onNextVisualChange', BX.proxy(this.onChangeForm, this)); // !!! Next is not a solution here
	...
	BX.loadScript('/bitrix/js/aspro.allcorp3/core_tree.js', function(){
		BX.ajax({
			timeout: 60,
			method: 'POST',
			dataType: 'html',
	...
	BX.loadCSS('/bitrix/css/aspro.allcorp3/catalog_cond.css');
	// BX.loadCSS(this.path + '/style.css?' + rand);
}

AsproAllcorp3CustomFilterControl.prototype =
{
	getPath: function()
	{
	...
	modifyCondValueGroup: function(ctrl)
	{
		...
		var propertyPrefix = 'CondIBProp',
			fieldCrossalesPrefix = 'CondCrossIBField',
			propertyCrossalesPrefix = 'CondCrossIBProp',
			allowedFields = [
				'CondIBXmlID', 'CondIBIBlock', 'CondIBCode', 'CondIBName', 'CondIBElement', 'CondIBSection', 'CondIBDateActiveFrom', 'CondIBDateActiveTo', 'CondIBSort', 'CondIBDateCreate', 'CondIBCreatedBy', 'CondIBTimestampX', 'CondIBModifiedBy',
		...
		for (var k in ctrl.children)
		{
			if (ctrl.children.hasOwnProperty(k))
			{
				...
				else
				{
					...
					if (
						(
							name[0] === propertyPrefix ||
							name[0] === propertyCrossalesPrefix ||
							name[0] === fieldCrossalesPrefix
						) &&
						name[2]
					)
					{
